...
$everyone = get_enrolled_users($context, '', 0, user_picture::fields('u').',u.aim'.',u.emailstop');  // add aim field for second email  hanna 13/7/15  // emailstop 16/12/15
...
foreach ($everyone as $userid => $user) {
    ...
    if ((!$globalaccess and !$mastercap) and
        empty($gids) or empty($filterd) or $userid == $USER->id) { //  show also sender? will drown in emails?  hanna 2/1/16
        continue;
    }
...
// array_unshift( $users, $USER); // hanna 13/4/16
...
if ($form->is_cancelled()) {
 ...
 //                   strip_tags($data->message), $data->message, $file, $filename);
  ...
  //              }
                if ($everyone[$userid]->emailstop == 0) {  // if emailstop  hanna 13/7/15
                    $hellohead = get_string('hellotoyou','core_davidson') . ' ' . $everyone[$userid]->firstname.' ' . '<br>' ;  // hanna added 8/6/14
                    //    if ($everyone[$userid]->lang == 'he' || $everyone[$userid]->lang == 'ar') {
                    if (current_language() == 'he' || current_language() == 'ar') {
                        $hellohead = '<div style="direction: rtl;text-align: right;">' . $hellohead . '</div>';  // aligned according to lang
                    }
                    $success = email_to_user($everyone[$userid], $user, $subject,
                        $hellohead . strip_tags($data->message), $hellohead . $data->message, $file, $filename);
                    //  add a second email address in aim in user profile
                    if ($data->parentmail) { // option to add parents' email
                        if (!empty ($everyone[$userid]->aim)) {
                            $tempemail = $everyone[$userid]->email;
                            $everyone[$userid]->email = $everyone[$userid]->aim;
                            email_to_user($everyone[$userid], $user, $subject,
                                $hellohead . strip_tags($data->message), $hellohead . $data->message, $file, $filename);
                            $everyone[$userid]->email = $tempemail;
                        }
                    }   // end if parentemail

                    if (!$success) {
                        $warnings[] = get_string("no_email", 'block_clampmail', array('firstname'=> $everyone[$userid]->firstname, 'lastname' => $everyone[$userid]->lastname));//$everyone[$userid]);
                    }
                } // end if emailstop
            ...
            if ($data->receipt) {
                ...
                $hellohead . strip_tags($data->message), $hellohead.$data->message, $file, $filename); // hanna 13/7/15
             }
