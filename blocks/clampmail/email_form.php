...
class email_form extends moodleform {
    ...
    public function definition() {
        ...
        $filters->text = html_writer::tag('div',
            ...
            array('id' => 'groups', 'multiple' => 'multiple', 'size' => 15))
        ) . html_writer::tag('div',
...
// groups size was 5  hanna 16/12/15
        ...
        // add option for second email for parents - radio1  hanna 13/7/15
        $radio1 = array(
            $mform->createElement('radio', 'parentmail', '', get_string('yes'), 1),
            $mform->createElement('radio', 'parentmail', '', get_string('no'), 0)
        );

        $mform->addGroup($radio1, 'parentmail_action', clampmail::_s('parentmail'), array(' '), false);
        $mform->addHelpButton('parentmail_action', 'parentmail', 'block_clampmail');
        $mform->setDefault('parentmail', !empty($config['parentmail']));
