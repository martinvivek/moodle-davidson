...
//echo $_SERVER['HTTP_REFERER'];die;
    ...
    $printreport = optional_param('print','',PARAM_BOOL);  //  nadavkav 25/3/14

    if (!$courseid) {
        if ($result = preg_match('/course\/view\.php\?id\=(.*)/', $_SERVER['HTTP_REFERER'], $matches)){
            $courseid = $matches[1];
        }
    }
    ...
    if ($printreport) {  //  nadavkav 25/3/14
        //$PAGE->set_pagelayout('printreport');
        $PAGE->set_pagelayout('print');
    } else {
        $PAGE->set_pagelayout('incourse_reports');
    }
...
//            echo '<style>  #weizmanndavidsonlogo {position: fixed !important; top: 0 !important;} </style>';
//            echo '<div id="weizmanndavidsonlogo" ><img src="'.$OUTPUT->pix_url('reports/logos', 'block_configurable_reports').'"></div>';
//            echo '<div id="weizmannlogosfull" ><img src="'.$OUTPUT->pix_url('reports/logosfull', 'theme').'"></div>';

            $reportcourse = $DB->get_record('course',array('id'=>$_GET['filter_courses']));
            $coursemetadata = $DB->get_record('coursemetadata_info_data', array('course' => $reportcourse->id, 'fieldid' => '3'));
            echo '<div id="reporttitle">'.$reportname.' - '.$reportcourse->fullname.' - '.$reportcourse->idnumber . ' ' . get_string("gmul",'block_configurable_reports') . ' = '.$coursemetadata->data.'</div>';
            echo  $report->summary . '<br>';
    ...
    if ($printreport) {   //  nadavkav 25/3/14
        echo '<div id="footerlogo"><img src="'.$OUTPUT->pix_url('reports/footer', 'block_configurable_reports').'"></div>';
    }