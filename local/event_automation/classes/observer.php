...
class observer {
    ...
    public static function record_updated(\mod_data\event\record_updated $event) {
        ...
        $to_testuser = $DB->get_record('user', array('id' => 7)); // nadavkav local userid = 2 pegasus1 userid = 7
        ...
        $to_user_moodle = \core_user::get_user(23183, '*', MUST_EXIST); // noga shabtai - stage 3

        if ($event->contextinstanceid == 19170) { // pegasus1
            //if ($event->contextinstanceid == 70) { // nadavkav local
            $data_field_id = 409; // pegasus1
            //$data_field_id = 6; // nadavkav local
