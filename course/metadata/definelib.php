...
class coursemetadata_define_base {
    ...
    function define_form(&$form) {
        $form->addElement('header', '_commonsettings', get_string('coursemetadatacommonsettings', 'admin'));
        $this->define_form_common($form);

        $form->addElement('header', '_specificsettings', get_string('coursemetadataspecificsettings', 'admin'));
    ...
    function define_form_common(&$form) {
        ...
        $form->addElement('text', 'shortname', get_string('coursemetadatashortname', 'admin'), 'maxlength="100" size="25"');
        $form->addRule('shortname', $strrequired, 'required', null, 'client');
        $form->setType('shortname', PARAM_ALPHANUM);

        $form->addElement('text', 'name', get_string('coursemetadataname', 'admin'), 'size="50"');
        $form->addRule('name', $strrequired, 'required', null, 'client');
        $form->setType('name', PARAM_MULTILANG);

        $form->addElement('editor', 'description', get_string('coursemetadatadescription', 'admin'), null, null);

        $form->addElement('selectyesno', 'required', get_string('coursemetadatarequired', 'admin'));

        $form->addElement('selectyesno', 'locked', get_string('coursemetadatalocked', 'admin'));

        $form->addElement('selectyesno', 'forceunique', get_string('coursemetadataforceunique', 'admin'));

        $form->addElement('selectyesno', 'coursecreate', get_string('coursemetadatashowoncreate', 'admin'));

        $choices = array();
        $choices[COURSEMETADATA_VISIBLE_NONE]    = get_string('coursemetadatavisiblenone', 'admin');
        $choices[COURSEMETADATA_VISIBLE_PRIVATE] = get_string('coursemetadatavisibleprivate', 'admin');
        $choices[COURSEMETADATA_VISIBLE_ALL]     = get_string('coursemetadatavisibleall', 'admin');
        $form->addElement('select', 'visible', get_string('coursemetadatavisible', 'admin'), $choices);
        $form->addHelpButton('visible', 'coursemetadatavisible', 'admin');
        $form->setDefault('visible', COURSEMETADATA_VISIBLE_ALL);

        $choices = coursemetadata_list_categories();
        $form->addElement('select', 'categoryid', get_string('coursemetadatacategory', 'admin'), $choices);
    }
    ...
    function define_validate_common($data, $files) {
        ...
        /// Check the shortname is unique
            if ($field and $field->id <> $data->id) {
                $err['shortname'] = get_string('coursemetadatashortnamenotunique', 'admin');
            }
...
function coursemetadata_move_field($id, $move) {
    ...
    if ($swapfield = $DB->get_record('coursemetadata_info_field', array('categoryid'=>$field->categoryid, 'sortorder'=>$neworder), 'id, sortorder')) {
...
function coursemetadata_list_datatypes() {
    ...
    $plugins = core_component::get_plugin_list('coursemetadatafield'); //get_plugin_list('coursemetadatafield');
...
function coursemetadata_edit_category($id, $redirect) {
    ...
    if ($categoryform->is_cancelled()) {
        ...
        if (empty($id)) {
            $strheading = get_string('coursemetadatacreatenewcategory', 'admin');
        } else {
            $strheading = get_string('coursemetadataeditcategory', 'admin', format_string($category->name));
        }
...
function coursemetadata_edit_field($id, $datatype, $redirect) {
    ...
    if ($fieldform->is_cancelled()) {
        ...
        if ($data = $fieldform->get_data()) {
            require_once($CFG->dirroot.'/course/metadata/field/'.$datatype.'/define.class.php');
