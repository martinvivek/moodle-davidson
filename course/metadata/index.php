...
require_once($CFG->dirroot.'/course/metadata/lib.php');
require_once($CFG->dirroot.'/course/metadata/definelib.php');

admin_externalpage_setup('coursemetadata');

$action   = optional_param('action', '', PARAM_ALPHA);

$redirect = $CFG->wwwroot.'/course/metadata/index.php';

$strchangessaved    = get_string('changessaved');
$strcancelled       = get_string('cancelled');
$strdefaultcategory = get_string('coursemetadatadefaultcategory', 'admin');
$strnofields        = get_string('coursemetadatanofieldsdefined', 'admin');
$strcreatefield     = get_string('coursemetadatacreatefield', 'admin');
...
switch ($action) {
    ...
    case 'deletefield':
        ...
        $strheading = get_string('coursemetadatadeletefield', 'admin');
        ...
        echo $OUTPUT->confirm(get_string('coursemetadataconfirmfielddeletion', 'admin', $datacount), $formcontinue, $formcancel);
        echo $OUTPUT->footer();
        die;
        break;
    case 'editfield':
...
/// Print the header
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('coursemetadata', 'admin'));

/// Check that we have at least one category defined
if ($DB->count_records('coursemetadata_info_category') == 0) {
...
foreach ($categories as $category) {
    ...
    $table->head  = array(get_string('coursemetadatafield', 'admin'), get_string('edit'));
...
$popupurl = new moodle_url('/course/metadata/index.php?id=0&action=editfield');
...
function coursemetadata_field_icons($field) {
    ...
    $datacount  = $DB->count_records('coursemetadata_info_data', array('fieldid'=>$field->id));
