...
com.wiris.quizzes.impl.HTMLTools.stripTags = function(html) {
	var e = new EReg("<[^>]*>","g");
...
com.wiris.quizzes.impl.HTMLTools.prototype = {
	...
	,unitTestDev: function() {
		haxe.Log.trace("compare(a,a1)=" + com.wiris.quizzes.impl.HTMLTools.compareStrings("a","a1"),{ fileName : "HTMLTools.hx", lineNumber : 2287, className : "com.wiris.quizzes.impl.HTMLTools", methodName : "unitTestDev"});
		var a = new Array();
		a.push("a");
		a.push("a1");
		...
		haxe.Log.trace("sort({a1,a})" + r,{ fileName : "HTMLTools.hx", lineNumber : 2298, className : "com.wiris.quizzes.impl.HTMLTools", methodName : "unitTestDev"});
	}
	,unitTestTables: function() {
		var variables = new Hash();
		var t = new Hash();
		t.set("T","{1,2,3,4}");
		t.set("M","{a,b,c,d}");
		t.set("r","10");
		variables.set(com.wiris.quizzes.impl.MathContent.TYPE_TEXT,t);
		var m = new Hash();
	...
	,unitTestExtractText: function() {
		...
		var outputs = ["Cap de les altres","La resposta és <math><mrow><mfrac><mfrac><mn>1</mn><mn>2</mn></mfrac><mi>x</mi></mfrac></mrow></math>.","<math><mrow><mo>(</mo></mrow></math>tiruliru<math><mrow><mo>)</mo></mrow></math>","<math><mrow><msqrt><mtext>radicand</mtext></msqrt></mrow></math>"," a b "," "];
		var i;
		var _g1 = 0, _g = inputs.length;
		while(_g1 < _g) {
			var i1 = _g1++;
	...
	,parseTabularVariableMathML: function(value) {
			...
			if(!closing) {
				...
				if(name == "mo" && !closing) {
					...
					if(op == "{" || op == "[" || op == "(") level++; else if(op == "}" || op == "]" || op == ")") level--; else if(op == this.separator && level == 0) {
						parts.push(this.addMathTag(HxOverrides.substr(value,lastindex,start - lastindex)));
						lastindex = end + 7;
					}
				}
			}
		...
		parts.push(this.addMathTag(HxOverrides.substr(value,lastindex,null)));
		return parts;
	}
	...
	,isTabularTextVariable: function(value) {
		var list = new EReg("[\\[\\{]([^\\" + this.separator + "]*\\" + this.separator + ")+[^\\}\\]]+[\\}\\]]","");
		return list.match(value);
	}
	,isTabularMathMLVariable: function(value) {
		var list = new EReg("(<math[^>]*>)?<mfenced(\\s+open\\s*=\\s*\"[\\[\\{]\"|\\s+close\\s*=\\s*\"[\\]\\}]\"){2}\\s*><mrow>([^\\" + this.separator + "]*<mo>\\" + this.separator + "</mo>)+[^\\" + this.separator + "]*</mrow></mfenced>(</math>)?","g");
		return list.match(value);
	}
	,isCellExpandable: function(td,variables) {
		var content = StringTools.trim(com.wiris.quizzes.impl.HTMLTools.stripTags(td));
		if(StringTools.startsWith(content,"#")) {
			content = HxOverrides.substr(content,1,null);
			if(td.indexOf("#" + content) != -1) {
				var v = variables.get(com.wiris.quizzes.impl.MathContent.TYPE_MATHML);
				if(v != null && v.exists(content) && this.isTabularMathMLVariable(v.get(content))) return true;
				v = variables.get(com.wiris.quizzes.impl.MathContent.TYPE_TEXT);
				if(v != null && v.exists(content) && this.isTabularTextVariable(v.get(content))) return true;
			}
		}
		return false;
	}
	...
	,replaceVariablesInsideHTMLTables: function(html,variables) {
		...
		while((tstart = html.indexOf("<table",tend)) != -1) {
			...
			var expanded = this.expandHorizontal(rows,grid,variables);
		...
		while(_g1 < _g) {
			...
			if(thisrow && j > 0) {
				...
				while(_g3 < _g2) {
					...
					var placeholder = StringTools.trim(com.wiris.quizzes.impl.HTMLTools.stripTags(model));
				...
				var prefix = HxOverrides.substr(original,0,original.indexOf("<td"));
				var suffix = HxOverrides.substr(original,original.lastIndexOf("</td>") + "</td>".length,null);
				var row = new StringBuf();
				var k;
				var _g2 = 0;
				while(_g2 < n) {
					var k1 = _g2++;
					row.b += Std.string(prefix);
	...
	,reconstructHorizontalExpand: function(rows,grid) {
		...
		while(_g1 < _g) {
			...
			var row = rows[2 * i1 + 1];
			newTable.b += Std.string(HxOverrides.substr(row,0,row.indexOf("<td")));
			var j;
			var _g3 = 0, _g2 = grid[i1].length;
			while(_g3 < _g2) {
				var j1 = _g3++;
			...
			newTable.b += Std.string(HxOverrides.substr(row,row.lastIndexOf("</td>") + "</td>".length,null));
		}
	...
	,expandHorizontal: function(rows,grid,variables) {
		...
		while(!end) {
			...
			while(j < grid.length && thiscolumn && !end) {
				var thiscell = false;
				if(i < grid[j].length) thiscell = this.isCellExpandable(grid[j][i],variables); else end = true;
			...
			if(thiscolumn && !end) {
				...
				while(_g1 < _g) {
					...
					var parsed = this.parseTabularVariable(HxOverrides.substr(placeholder,1,null),variables);
					var k;
					var _g3 = 0, _g2 = parsed.length;
					while(_g3 < _g2) {
						var k1 = _g3++;
						sb.b += Std.string(prefix);
						sb.b += Std.string(parsed[k1]);
	...
	,parseTableCells: function(rows) {
		...
		while(i < rows.length) {
			...
			var tdstart;
			var tdend = 0;
			while((tdstart = row.indexOf("<td",tdend)) != -1) {
				tdend = row.indexOf("</td>",tdstart);
				if(tdend == -1) tdend = row.length; else tdend += "</td>".length;
	...
	,expandVariables: function(html,variables) {
		...
		while(_g1 < _g) {
			...
			if(StringTools.startsWith(token,"<math")) {
				...
				if(v != null) token = this.replaceVariablesInsideHTML(token,v,com.wiris.quizzes.impl.MathContent.TYPE_IMAGE);
