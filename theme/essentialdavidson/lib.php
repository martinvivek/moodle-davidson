...
// Using the following function to display fake custom navigation block
// (specific to Davidson's needs. together with special course/format/grid layout settings)
function theme_essentialdavidson_get_html_for_settings(renderer_base $output, moodle_page $page, $fakeblockregion) {

    global $CFG, $COURSE, $USER;

    if (($page->pagelayout == 'quiz-attempt' OR $page->pagelayout == 'incourse')
        AND !empty($page->activityrecord->id)) {

        $bc = new block_contents();
        $bc->title = get_string('toc', 'theme_essentialdavidson');
        $bc->attributes['class'] = 'block block_course_module_nav';

        $coursemodules = new course_modinfo($COURSE, $USER->id);
        $modules = $coursemodules->get_cms();
        ...
        if (isset($_GET['navsection'])) {
            ...
            if (!empty($_COOKIE["navsection"])) {
                $navsection = $_COOKIE["navsection"];
            } else {
                $navsection = '';
            }
        }
        if (isset($_GET['navfmodid'])) {
            ...
            if (!empty($_COOKIE["navfmodid"])) {
                $navfmodid = $_COOKIE["navfmodid"];
            } else {
                $navfmodid = '';
                //print_object($modules);die;
                if (empty($modules)) exit;
                foreach ($modules as $mod) {
                    if (empty($page->activityrecord->id)) continue;
                    if ($mod->instance == $page->activityrecord->id) {
                        $navfmodid = $mod->id;
                        //print_object($mod);
                        if (empty($navsection)) $navsection = $mod->sectionnum;
                    }
                }
            }
        }
        */
        // Alternative to saving current mod id with cookies
        $navsection = $page->cm->sectionnum;
        $navfmodid = $page->cm->id;

        $nav = '<ul>';

        //  previous "button was here   hanna 21/10/15

//print_object($page->cm);

        // Current section title.
        $sectioninfo = $coursemodules->get_section_info($navsection);
        //    $nav .= '<li class="currentsection">'.'יחידה נוכחית: '.theme_essentialdavidson_ltrim_hidemechar($sectioninfo->name, '^').'</li>';
        $nav .= '<li class="currentsection">'.get_string('currentsection', 'theme_essentialdavidson') .': '.theme_essentialdavidson_ltrim_hidemechar($sectioninfo->name, '^').'</li>';
        // List of all visible modules + highlight current/selected activity/resource.
        foreach ($modules as $mod) {
            $modinfo = get_fast_modinfo($COURSE->id)->cms[$mod->id];
            if ($mod->sectionnum != $navsection) continue;
            if ($mod->modname == 'label') continue; // Do not display Labels
            $icon = new pix_icon($modinfo->icon, get_string('modulename', $modinfo->modname), $modinfo->iconcomponent);
            /*if (empty($icon->pix)) {   $icon_pix = 'f/html-24';
                } else {       $icon_pix = $icon->pix;       }*/
            require_once($CFG->dirroot .'/course/lib.php');
            $modname = theme_essentialdavidson_ltrim_hidemechar($modinfo->name, '*');
            $activitylink = html_writer::empty_tag('img', array('src' => $mod->get_icon_url(),
                    'class' => 'iconlarge activityicon', 'alt' => ' ', 'role' => 'presentation')) . $accesstext='' .
                    html_writer::tag('span', $modname . $altname='', array('class' => 'instancename'));
            if (isset($navfmodid) AND $navfmodid == $mod->id) {
                $isselected = 'selected';
            } else {
                $isselected = '';
            }
            if ($mod->uservisible and mb_substr($modinfo->name, 0, 1) != '*') {
                $nav .= '<li class="'.$isselected.'">'.html_writer::link($mod->url.'&navsection='.$navsection.'&navfmodid='.$modinfo->id,
                        $activitylink, array('class' => $linkclasses='', 'onclick' => $onclick='')) .
                    $groupinglabel=''."</li>";
            }
            //$nav .= '<li><img src="'.$CFG->wwwroot.'/pix/'.$icon_pix.'.png">'.mb_substr($modinfo->name, 0, 20).'...</li>';
        }

        $modinfo = get_fast_modinfo($COURSE->id);  // set modinfo again since it's changed
        // Previous section link to first activity in that section.
        if (isset($navsection) AND $navsection > 0) {
            //$nav .= '<li class="previoussection">'.$this->theme_essentialdavidson_ltrim_hidemechar($sectioninfo->name, '^').'</li>';
            $previoussection = $coursemodules->get_section_info($navsection-1);
            $previoussectionmodnumbers = $modinfo->sections[$previoussection->section];
            $previoussection_firstmodinfo = $modinfo->cms[$previoussectionmodnumbers[0]];
            if ($previoussection_firstmodinfo->uservisible) {
                if (current_language() == 'en'){
                    $nav .= '<li class="previoussection"><i class="fa fa-arrow-left"></i>&nbsp;';
                } else {
                    $nav .= '<li class="previoussection"><i class="fa fa-arrow-right"></i>&nbsp;';
                }
                $nav .= html_writer::link($previoussection_firstmodinfo->url.'&navsection='.($navsection-1).'&navfmodid='.$previoussection_firstmodinfo->id,
                    get_string('previoussection', 'theme_essentialdavidson').': '.theme_essentialdavidson_ltrim_hidemechar($previoussection->name, '^'),
                    array('class' => $linkclasses='', 'onclick' => $onclick=''));
                $nav .= '</li>';
            }
        }   //  end of previous" button


        // Next section link to first activity in that section.
        $modinfo = get_fast_modinfo($COURSE->id);
        if (isset($navsection) AND $navsection > 0) {
            //$nav .= '<li class="nextsection">'.$this->theme_essentialdavidson_ltrim_hidemechar($sectioninfo->name, '^').'</li>';
            $nextsection = $coursemodules->get_section_info($navsection+1);
            if (isset($nextsection->section) and isset($modinfo->sections[$nextsection->section])) {
                $nextsectionmodnumbers = $modinfo->sections[$nextsection->section];
                $nextsection_firstmodinfo = $modinfo->cms[$nextsectionmodnumbers[0]];
            }
            if (isset($nextsection_firstmodinfo) AND $nextsection_firstmodinfo->uservisible
                AND !empty($nextsection_firstmodinfo->url)) {
                $nav .= '<li class="nextsection">';
                $nav .= html_writer::link($nextsection_firstmodinfo->url.'&navsection='.($navsection+1).'&navfmodid='.$nextsection_firstmodinfo->id,
                    get_string('nextsection', 'theme_essentialdavidson').': '.theme_essentialdavidson_ltrim_hidemechar($nextsection->name, '^'), array('class' => $linkclasses='', 'onclick' => $onclick=''));
                if (current_language() == 'en') {
                    $nav .= '&nbsp;<i class="fa fa-arrow-right"></i></li>';
                }  else {
                    $nav .= '&nbsp;<i class="fa fa-arrow-left"></i></li>';
                }
            }
        }

        // todo: do we need this? (title is already linked)
        // Back to course link
        //$nav .= html_writer::tag('li',html_writer::link(new moodle_url($CFG->wwwroot.'/course/view.php',array('id'=>$COURSE->id)),
        //            'בחזרה לעמוד הקורס הראשי', array('class' => $linkclasses='', 'onclick' => $onclick='')), array('class'=>'backtocourse'));

        $nav .= html_writer::tag('li',html_writer::link(new moodle_url($CFG->wwwroot.'/course/view.php',array('id'=>$COURSE->id)),
            get_string('backtocoursepage', 'theme_essentialdavidson'), array('class' => $linkclasses='', 'onclick' => $onclick='')), array('class'=>'backtocourse'));

        $nav .= '</ul>';

        $bc->content = $nav;

        //$fakeblockregion = 'side-post';//$PAGE->blocks->get_default_region();
        $page->blocks->add_fake_block($bc, $fakeblockregion);
    }
}

function theme_essentialdavidson_ltrim_hidemechar($name, $hidemechar) {
    if (mb_substr($name, 0, 1) == $hidemechar) {
        return ltrim($name, $hidemechar);
    } else {
        return $name;
    }
}
 ...
 * @return bool.
 */
function theme_essentialdavidson_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;
    if (empty($theme)) {
    ...
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        if ($filearea === 'logo') {
            ...
            theme_essentialdavidson_serve_css($args[1]);
...
function theme_essentialdavidson_serve_css($filename) {
    global $CFG;
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_essentialdavidson_send_unmodified($lastmodified, $etagfile);
    }
    theme_essentialdavidson_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}

function theme_essentialdavidson_send_unmodified($lastmodified, $etag) {
...
function theme_essentialdavidson_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
...
function theme_essentialdavidson_process_css($css, $theme) {
    global $PAGE;
    $outputus = $PAGE->get_renderer('theme_essentialdavidson', 'core');
