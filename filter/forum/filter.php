...
/*
// Import of css file
if (file_exists($CFG->dirroot.'/theme/'.$PAGE->theme->name.'/styles.css')) {
	echo '<style type="text/css">';
    echo '	@import url('. $CFG->httpsthemewww.'/'.$PAGE->theme->name.'/styles.css);';
	echo '</style>';
} else {
	//echo '<style type="text/css">';
    //echo '	@import url('.$CFG->wwwroot.'/filter/forum/styles.css);';
	//echo '</style>';
}
*/
/**
* This filter shows a top "x" forum from a forum
 ...
 */
class filter_forum extends moodle_text_filter {
		...
		if ($regs[4]>0) {
			...
			if ($data = $DB->get_record('course_modules', array( 'id'=> $cmid, 'module'=> $moduleid->id)) ){
				...
				if ($discussions) {
					...
					$forum .= '<div class="filter_forum"><ul>';
...
//                            $forum .= print_user_picture($user->id, $COURSE->id, $user->picture, '16', true, true, '', false); //  'class'=>'profilepicture' ???
						...
						$forum .= substr($discussions[$i]["name"],0,$nbcaract);
