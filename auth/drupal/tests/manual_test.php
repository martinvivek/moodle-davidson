...
global $CFG;
require_once($CFG->dirroot.'/auth/drupal/auth.php');

/**
 * drupal authentication tests class.
 *
 * @package    auth_drupal
 * @category   test
 * @copyright  2014 Gilles-Philippe Leblanc <gilles-philippe.leblanc@umontreal.ca>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class auth_drupal_testcase extends advanced_testcase {

    /** @var auth_plugin_drupal Keeps the authentication plugin. */
    ...
    protected function setUp() {
        ...
        $this->authplugin = new auth_plugin_drupal();
        ...
        $this->authplugin->config = get_config(auth_plugin_drupal::COMPONENT_NAME);
    }
    ...
    public function test_user_update_password() {
        ...
        $this->assertGreaterThanOrEqual($expectedtime, get_user_preferences('auth_drupal_passwordupdatetime', 0, $user->id));
    ...
    public function test_process_config() {
        ...
        $config = get_config(auth_plugin_drupal::COMPONENT_NAME);
