...
if(!has_capability('mod/unicko:viewrecording', $context)) {
    $url = new moodle_url('/mod/unicko/view.php', array('id' => $cm->id));
    redirect($url);
}

//add_to_log($course->id, 'unicko', 'viewrecording', "manage.php?id={$cm->id}", $unicko->name, $cm->id);

/// Print the page header

$PAGE->set_context($context);
$PAGE->set_url('/mod/unicko/recordings.php', array('id' => $cm->id));
...
echo $OUTPUT->heading('Redirecting to the room recordings');
...
echo unicko_get_recordings_form($data);
