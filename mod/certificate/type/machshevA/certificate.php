...
/**
 * letter_non_embedded_beaver certificate type
 *
 * @package    mod
...
certificate_print_text($pdf, $x, $y + 115, 'C', 'freesans', '', 14, fullname($USER));
certificate_print_text($pdf, $x, $y + 151, 'C', 'freesans', '', 14, $USER->institution );
...
if (is_array($groups)) {
    ...
    //print_object(explode('_', $usergroup->name));
    if (count(explode('_', $usergroup->name)) == 3) {
        list($city, $school, $shchva) = explode('_', $usergroup->name);
        $school = str_replace('-', ' ', $school);
    } else {
        echo "Error: bad group name";
        $city = 'City';
        $school = 'School';
        $shchva = 'Shichva';
    }
...
//certificate_print_text($pdf, $x, $y + 205, 'C', 'freesans', '', 20, $course->fullname);
//certificate_print_text($pdf, $x, $y + 255, 'C', 'freesans', '', 14, certificate_get_date($certificate, $certrecord, $course));
